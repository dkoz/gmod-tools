...
local enableTicker = CreateClientConVar("gmod_enableticker", "1", true, false)
local textX
local currentMessage = 1
local messages = {
    "example message here!", -- Example of message that will appear on top of screen
    "whatever else here.",
}
...
hook.Add("HUDPaint", "gmod_Ticker", function()
    if (enableTicker:GetInt() > 0) then
        surface.SetFont("gmod_TickerFont")
        ...
        render.SetScissorRect(scrW * 0.25, 5, scrW * 0.75, 24, true)
            draw.SimpleText(currentText, "gmod_TickerFont", textX, 17, Color(currentColor.r, currentColor.g, currentColor.b, alpha), 0, 1)
